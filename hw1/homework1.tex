...
So, there is one state transition based on $\C$.
...
The new construct is the $\case$ command and the list primitive. We extend the
...
Thus, we can extend the semantic function thus :
...
list. Thus, this defines the denotational semantics of the $\case$ command.
...
We can define the translation function thus:
...
\then \; \C_1 \; \elsesym \; \C_2 \;$ statements, i.e.
...
argue that the translation function preserves denotational semantics.
...
No, in this case, the only difference is that $\LL : Expr \rightharpoonup \textbf{Com} $.
So, we evaluate the expressions in the list during comparison in the $\case$
command, i.e.
